<div class="main-container">
    <div id="banner-gallery" class="banner">
        <div class="wrapper">
            <h1 class="banner-name">
                GALLERY
            </h1>
        </div>
    </div>
    <div class="main wrapper clearfix">
        <div class="project-box">
            <h2>Domingo</h2>
            <div id="prj-1" class="project-img-main"></div>
            <div>
                <div id="prj-2" class="project-img-thumb"> 
                </div><div id="prj-3" class="project-img-thumb">
                </div><div id="prj-4" class="project-img-thumb">
                </div><div id="prj-5" class="project-img-thumb"></div>
            </div>
            <p></p>
        </div><div class="project-box">
            <h2>Virgil</h2>
            <div id="prj-6" class="project-img-main"></div>
            <div>
                <div id="prj-7" class="project-img-thumb"> 
                </div><div id="prj-8" class="project-img-thumb">
                </div><div id="prj-9" class="project-img-thumb">
                </div><div id="prj-10" class="project-img-thumb"></div>
            </div>
            <p></p>
        </div><div class="project-box">
            <h2>Residence</h2>
            <div id="prj-11" class="project-img-main"></div>
            <div>
                <div id="prj-12" class="project-img-thumb"> 
                </div><div id="prj-13" class="project-img-thumb">
                </div><div id="prj-14" class="project-img-thumb">
                </div><div id="prj-15" class="project-img-thumb"></div>
            </div>
            <p></p>
        </div>
    </div> <!-- #main -->
</div> <!-- #main-container -->

